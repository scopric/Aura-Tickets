-- ============================================================
-- AURA PLATFORM — Schema Completo (P0 + P1)
-- Sem integração de pagamento (Stripe/Woovi)
-- Execute no SQL Editor do Supabase (Nova Query)
-- ============================================================
...
-- 2.7 payments (schema apenas, sem gateway ativo)
...
-- 5.1 Trigger: criar perfil após signup
...
-- 5.6 Function: refresh event_summary
...
CREATE POLICY "Producers can CRUD own events" ON public.events
...
CREATE POLICY "Ticket types readable by event owner and public" ON public.ticket_types
  FOR SELECT USING (
    is_active = TRUE AND
    EXISTS (SELECT 1 FROM public.events WHERE id = event_id AND status = 'published')
    OR EXISTS (SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid())
  );
DROP POLICY IF EXISTS "Producers can manage own ticket types" ON public.ticket_types;
CREATE POLICY "Producers can manage own ticket types" ON public.ticket_types
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
...
CREATE POLICY "Producers can view event tickets" ON public.tickets
  FOR SELECT USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
...
CREATE POLICY "Producers can view event orders" ON public.orders
  FOR SELECT USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
...
DROP POLICY IF EXISTS "Producers can view event order items" ON public.order_items;
CREATE POLICY "Producers can view event order items" ON public.order_items
  FOR SELECT USING (EXISTS (
...
CREATE POLICY "Producers can manage own coupons" ON public.coupons
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
...
CREATE POLICY "Producers can view event check_ins" ON public.check_ins
  FOR SELECT USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
DROP POLICY IF EXISTS "Operators can create check_ins" ON public.check_ins;
CREATE POLICY "Operators can create check_ins" ON public.check_ins
...
CREATE POLICY "Producers can manage own seating maps" ON public.seating_maps
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));

-- 6.11 producer_balances
...
CREATE POLICY "Producers can view own balance" ON public.producer_balances
  FOR SELECT USING (auth.uid() = producer_id);
DROP POLICY IF EXISTS "Admins can manage all balances" ON public.producer_balances;
CREATE POLICY "Admins can manage all balances" ON public.producer_balances
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'
  ));
...
CREATE POLICY "Event team can manage tasks" ON public.tasks
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
...
CREATE POLICY "Producers can manage event customers" ON public.customers
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
...
CREATE POLICY "Producers can manage event communications" ON public.communications
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
...
CREATE POLICY "Producers can manage event certificates" ON public.certificates
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
...
CREATE POLICY "Producers can manage own templates" ON public.certificate_templates
  FOR ALL USING (auth.uid() = producer_id);

-- 6.17 affiliates
ALTER TABLE public.affiliates ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Anyone can read active affiliates" ON public.affiliates;
CREATE POLICY "Anyone can read active affiliates" ON public.affiliates
  FOR SELECT USING (is_active = TRUE);
DROP POLICY IF EXISTS "Producers can manage own affiliates" ON public.affiliates;
CREATE POLICY "Producers can manage own affiliates" ON public.affiliates
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
...
CREATE POLICY "Anyone can view gallery" ON public.event_gallery
  FOR SELECT USING (TRUE);
DROP POLICY IF EXISTS "Producers can manage own gallery" ON public.event_gallery;
CREATE POLICY "Producers can manage own gallery" ON public.event_gallery
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.events WHERE id = event_id AND producer_id = auth.uid()
  ));
...
CREATE POLICY "Producers can view own withdrawals" ON public.withdrawals
  FOR SELECT USING (auth.uid() = producer_id);
...
-- ============================================================
-- 7. DADOS INICIAIS (Seed Manual)
-- ============================================================
-- NOTE: Crie os usuários via Supabase Auth (painel ou API).
-- Os perfis serão criados automaticamente pelo trigger handle_new_user.
-- Depois, execute o seed.sql para criar eventos, ingressos, etc.

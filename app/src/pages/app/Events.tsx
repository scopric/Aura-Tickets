  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          <h1 className="font-serif text-4xl text-espresso">Descubra Eventos</h1>
          <p className="text-espresso/50 mt-2 text-base">
            Encontre os melhores eventos perto de você
          </p>
        </div>
        <div className="relative w-full lg:w-96">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-espresso/30" />
          <input
            type="text"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Buscar por nome, cidade ou categoria..."
            className="w-full pl-12 pr-4 py-3 bg-white/70 border border-espresso/10 rounded-2xl text-base text-espresso placeholder:text-espresso/30 focus:outline-none focus:ring-2 focus:ring-plum/20 focus:border-plum/30 transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
        <Filter className="w-4 h-4 text-espresso/30 flex-shrink-0 mr-1" />
        {categories.map((cat) => (
          <button
            key={cat}
            ...
            className={cn(
              ...
              activeCategory === cat
                ...
                : 'bg-white/60 text-espresso/60 hover:bg-white hover:text-espresso border border-espresso/5'
            )}
          >
            {cat}
          </button>
      ...
      {rest.length === 0 && !featured ? (
        <div className="text-center py-24">
          <div className="w-20 h-20 rounded-full bg-espresso/5 flex items-center justify-center mx-auto mb-6">
            <Star className="w-10 h-10 text-espresso/15" />
          </div>
          <h3 className="font-serif text-xl text-espresso mb-2">Nenhum evento encontrado</h3>
          <p className="text-espresso/40 text-base">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {rest.map((event) => (
              <div
                key={event.id}
                className="group rounded-2xl bg-white/60 border border-white/60 hover:border-plum/15 hover:shadow-elevated transition-all overflow-hidden flex flex-col"
              >
                {/* Image */}
                <div className="relative h-48 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="px-2.5 py-1 rounded-lg bg-white/90 backdrop-blur text-xs font-medium text-espresso">
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      favorites.has(event.id)
                        ...
                        : 'bg-white/70 text-espresso/40 hover:text-red-500'
                    )}
                  >
                ...
                <div className="p-5 flex flex-col flex-1">
                  <Link
                    ...
                    className="font-semibold text-espresso text-base group-hover:text-plum transition-colors line-clamp-1"
                  >
                    {event.title}
                  </Link>
                  <p className="text-sm text-espresso/40 mt-1 line-clamp-2 flex-1">
                  ...
                  <Link
                    to={`/event/${event.slug || event.id}`}
                    className="mt-4 inline-flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-plum/10 text-plum text-sm font-medium hover:bg-plum hover:text-cream transition-all"
                  >
                    Ver detalhes
                    <ArrowRight className="w-4 h-4" />
                  </Link>
      ...
      {/* Bottom CTA */}
      <div className="rounded-2xl bg-gradient-to-r from-plum/10 to-plum/5 border border-plum/10 p-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          <h3 className="font-serif text-xl text-espresso">Não encontrou o que procura?</h3>
